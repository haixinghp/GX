...
static void aes_key_expansion(AES_CYPHER_T mode, uint8_t *key, uint8_t *round)
{
    uint32_t *w = (uint32_t *)round;
    uint32_t  t;
    int      i = 0;
    #ifdef AES_LOG_ENABLE
...
static void inv_shift_rows(AES_CYPHER_T mode, uint8_t *state)
{
    uint8_t *s = (uint8_t *)state;
    int i, j, r;
...
static void inv_sub_bytes(AES_CYPHER_T mode, uint8_t *state)
{
    int i, j;
...
static void inv_mix_columns(AES_CYPHER_T mode, uint8_t *state)
{
