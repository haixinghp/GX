...
extern unsigned int g_song_spiaddr;		//当前播放的音频文件的SPI-Flash地址														
extern UINT32 link_index;
extern UINT32 link_index_max;
...
/*******************************************************************************
* Function Name  : DMA1_IRQHandler
* Description    : dma中断清除函数
* Input          : None
*
* Output         : None
* Return         : None
******************************************************************************/
void DMA_ClrInt(void)
{
...
void SSI_SetNum(SSI_TypeDef *SSIx, int num)
{
...
void SSI_DisDma(SSI_TypeDef *SSIx)
{
	SSIx->SSI_DMACR=0X00;
}
//#endif														
...
* Return         : None
******************************************************************************/
UINT8 unSesorTouEvent;
extern UINT32 play_song_compete;
extern UINT8 Single_TranDma;
...
void DMA2_IRQHandler(void)
{
	...
	if(m_dma_control->DMA_STATTFR & 0x0f)//dma done
	{
		...
		if(link_index == (link_index_max-1)||Single_TranDma==1)
		{
	...
	//NO XIP mode
	if(m_dma_control->DMA_STATBLOCK & 0x0f)//dma error
	{
		
		if(unSesorTouEvent == 1)
		{
...
m_dma_channel[n]->DMA_CFG_HIGH = (8<<11)|(2<<7);
	m_dma_control->DMA_MASKBLOCK=CHANNEL_UMASK(n);		//每个link传输完的中断
