...
#define C0_WKPPADCR  *(volatile unsigned int *)(0x081400c8)
/**
* @brief SubC bin
* SubC0_HandleTypeDef
* 1. 增加了C0 gint中断唤醒功能
...
uint32_t drv_SubC0_init(SubC0_HandleTypeDef *hSubC0)
{
    ...
    NVIC_Init(3, 3, DCMI_IRQn, 2); //使能C0唤醒IO中断
...
//    CPM->CPM_PADWKINTCR |= (1u << 15); //可能会有死机出现
...
void C0_WakeUp_Low(void)
{
    
    C0_WKPPADCR &= ~(0x1<<13);
    
}
void C0_WakeUp_Init(void)
{
    C0_WKPPADCR |= 0x1<<9;
    C0_WKPPADCR &= ~(0x1<<1);
    C0_WKPPADCR |= (0x1<<13);
