...
void Demo_UART_GpioConfig(UART_TypeDef *UARTx)
{

	UART_ConfigGpio(UARTx,UART_RX, GPIO_OUTPUT);
	UART_ConfigGpio(UARTx,UART_TX, GPIO_INPUT);
	
	while(1)
	{
		UART_WriteGpioData(UARTx,UART_RX, UART_ReadGpioData(UARTx,UART_TX));
		DelayMS(100);
	}
...
void Demo_UART_CPUIntTransfer(UART_TypeDef *UARTx)
{
    ...
    while(1)
    {
        ret = UART_RecvByte(UARTx, &uart_data_buf[i]);
        if(STATUS_OK == ret)
        {
            i ++;
        }
        else if(STATUS_NULL == ret)
        {
            if(0 != i)
            {
...
//    Demo_UART_PinSwap(uart_type_id);
	...
	//UART pin配置普通GPIO功能，不支持中断
//	Demo_UART_GpioConfig(uart_type_id);
